  ...
  return (
    <header
      className={`sticky top-0 z-50 w-full transition-all duration-300 ${scrolled
          ? 'bg-background/85 backdrop-blur-xl border-b border-border/60 shadow-sm shadow-black/20'
          : 'bg-transparent border-b border-transparent'
        }`}
    >
      {/* Inner wrapper: 3-column grid so nav is TRULY centered in the page */}
        ...
        <div className="flex items-center">
          <Link href="/" className="group inline-flex">
            <Image
              ...
              width={200}
              height={52}
              className="object-contain h-11 w-auto transition-opacity duration-200 group-hover:opacity-80"
              priority
            />
          </Link>
        </div>
